...
**1️⃣ Download the `spl_unlock_linux` package** from [Releases](https://github.com/KiMiGuel/Cubot-KingKong-ES3-Unisoc-T615-Root/releases) and unzip. Everything's inside: `spd_dump` (Linux/libusb build with the reconnect fix), `gen_spl-unlock`, `chsize`, sources + Makefile, `fdl1-dl.bin`, `fdl2-dl.bin`, `fdl2-cboot.bin`, `misc-wipe.bin`.
...
**3️⃣ Verify you built the right thing** — two checks, both must pass:
...
**4️⃣ One file is NOT in the package:** `init_boot.img` — extract it from **your own** stock PAC (build above). Stay inside the package folder the whole time — dumps land in the current directory.

---

## 💾 Phase 2 — Back up your stock loaders

**5️⃣ Enter BROM:** power off → hold **Volume Down** → plug USB → keep holding. Confirm:

```
lsusb | grep -i '1782:4d00'
```

**6️⃣ Dump `splloader` + `uboot`:**
...
**8️⃣ Re-enter BROM (step 5). Write the working FDL2:**
...
**9️⃣ Re-enter BROM. Run the unlock:**
...
- ⏱️ `timeout reached` after that — **this is what you WANT.** Not an error. Do **not** re-run; move to the next step. (Only if you never see `CMD_CONNECT FDL1`, run it once more.)
...
Check `m.bin`: 64 zero bytes = still locked (repeat step 9) · 32-byte string + two 16-byte hashes = **unlocked** ✅. A trailing timeout here is normal.

**1️⃣1️⃣ Restore the stock loaders + trigger the required wipe:**
...
**1️⃣2️⃣ Patch `init_boot`:** boot Android → Settings → About Phone → tap Build Number ×7 → Developer Options → **USB debugging**. Then:

```
adb push init_boot.img /sdcard/Download/
```

In the Magisk app: **Install → Select and Patch a File → `init_boot.img`**, then pull it back:

```
adb pull /sdcard/Download/magisk_patched-*.img .
```

**1️⃣3️⃣ Flash it — in fastbootd, not the plain bootloader:**
...
| Boot loop / soft brick at any point | BROM (step 5) always answers — reflash stock firmware, redo the guide |
